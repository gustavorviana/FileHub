...
{
    /// <summary>
    /// Read-only stream over an S3 object. Reads are chunked into ranged
    /// <c>GetObject</c> requests (10 MB per range) so any object size
    /// streams without loading fully into memory. Seekable; length comes
    ...
    /// </summary>
    internal sealed class S3ReadStream : S3FileStreamBase
    {
        ...
        public S3ReadStream(AmazonS3File file)
        {
        ...
        {
            if (_disposed) throw new ObjectDisposedException(nameof(S3ReadStream));
        }
    }
}
